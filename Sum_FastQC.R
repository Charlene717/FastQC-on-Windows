...
parse_fastqc_summary <- function(zip_file) {
  # 解壓縮並讀取 summary.txt
  temp_dir <- tempdir()
  unzip(zip_file, files = "*/summary.txt", exdir = temp_dir)
  
  # 找到 summary.txt 的實際路徑
  ...
  # 讀取 summary.txt
  summary_data <- read.delim(summary_file, header = FALSE, sep = "\t", stringsAsFactors = FALSE)
...
# 將整理後的結果輸出
write.csv(fastqc_summary_df, file = "FastQC_summary.csv", row.names = FALSE)
